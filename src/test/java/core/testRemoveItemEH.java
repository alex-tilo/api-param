	...
	public class testRemoveItemEH {
		...
		@Test(dataProvider = "scenarios", dataProviderClass = DataFeed.class,enabled = true, priority = 1, description = "API Validation removeItem")
		@Description(
						"<b>URI: </b> https://retail-api.azure-api.net/ <br />" + 
						"<b>Port: </b> 443 <br />" +
						"<b>Environment: </b> QA [scanandgoQaJS] <br />"
					)
		public void schemaRemoveItemEH(String testcase) throws Exception {
			
			String scenario = testcase;
			setup(config, environment);
		    try {
			response = given()
					.headers(u.readJSONFileAsMap(path, "header_"+ scenario))
					.header("Ocp-Apim-Subscription-key", u.getKey("Ocp-Apim-Subscription-key"))	
					.header("oktatoken", u.getToken("access_token"))	
					.body(u.readJSONFile(path, "payload_" + scenario + ".json"))
			...
			response.then()
					...
					.and()
					.body(matchesJsonSchemaInClasspath(path + "/schema_" +scenario+".json"))
					.extract().response();
		    }
