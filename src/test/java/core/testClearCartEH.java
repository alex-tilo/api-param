...
public class testClearCartEH {
	...
	@Test(dataProvider = "scenarios", dataProviderClass = DataFeed.class, enabled = true, priority = 1, description = "API Validation clearCart")
	@Description("<b>URI: </b> https://retail-api.azure-api.net/ <br />" + "<b>Port: </b> 443 <br />"
			+ "<b>Environment: </b> QA [scanandgoQaJS] <br />")
	public void schemaClearCartEH(String testcase) throws Exception {
		...
		try {
			...
			response.then().log().headers().log().body().assertThat()
					...
					// charset=utf-8").and().header("Content-Encoding", "gzip")
					.and()
					// .header("Request-Context", containsString("appId=cid-v1:")).and()
					.body(matchesJsonSchemaInClasspath(path + "/schema_" + scenario + ".json")).extract().response();
		} catch (Exception e) {
			e.printStackTrace();
		}
